...
class AgentArrivedInterventionTest {

  private AgentArrivedIntervention agentArrivedIntervention;
  ...
  @BeforeEach
  void setUp() {
    agentArrivedIntervention = new AgentArrivedIntervention(validTime, zoneId, interventionId);
  }

  @Test
  void givenBuildingAndInterventionId_whenHandleEvent_thenReturnAgentArrivedActions() {
    List<Action> expectedActions = List.of(returnedAction);
    when(building.handleAgentArrived(eventContext, interventionId)).thenReturn(expectedActions);

    List<Action> result = agentArrivedIntervention.handleEvent(building, eventContext);
