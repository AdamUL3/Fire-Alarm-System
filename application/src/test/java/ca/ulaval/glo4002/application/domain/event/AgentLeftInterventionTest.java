...
class AgentLeftInterventionTest {

  private AgentLeftIntervention agentLeftIntervention;
  ...
  @BeforeEach
  void setUp() {
    agentLeftIntervention = new AgentLeftIntervention(validTime, zoneId, interventionId);
  }

  @Test
  void givenBuildingAndInterventionId_whenHandleEvent_thenReturnAgentLeftActions() {
    List<Action> expectedActions = List.of(returnedAction);
    when(building.handleAgentLeft(eventContext, interventionId)).thenReturn(expectedActions);

    List<Action> result = agentLeftIntervention.handleEvent(building, eventContext);
