...
class FirePreAlarmTest {
  private FirePreAlarm firePrealarm;
  ...
  @BeforeEach
  void setUp() {
    firePrealarm = new FirePreAlarm(validTime, zoneId);
  }

  @Test
  void givenBuilding_whenHandleEvent_thenReturnFirePreAlarmActions() {
    List<Action> actionsReturned = List.of(returnedAction);
    when(building.handlePreAlarm(eventContext)).thenReturn(actionsReturned);

    List<Action> actions = firePrealarm.handleEvent(building, eventContext);
